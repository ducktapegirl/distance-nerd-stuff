...
and the orchestrator writes it into `Project Docs/Specs/strava-data/dashboard-spec.md`. You never write build code.
...
to `Project Docs/Specs/strava-data/dashboard-spec.md` (the orchestrator writes it). For each new view, emit a
block:
```
### {Chart name}
