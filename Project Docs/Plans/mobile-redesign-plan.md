...
The Strava dashboard (`strava-data/dashboard/`) and Running Log dashboard (`running-log/dashboard/`) were previously made "responsive" (CSS grid breakpoints, `responsive: true` on Plotly, a viewport meta tag) but that only covers generic reflow, not a real mobile experience. On an actual phone today:
...
- `running-log/dashboard/template.py` — same categories of changes as Strava's `template.py`, applied independently (including removing its existing narrow detail-panel mobile override at the old 600px breakpoint).
