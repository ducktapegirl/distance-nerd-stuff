...
`running-log/college.html` (CSS 433-498, HTML 1006-1164) and `running-log/visualize_log.py`
...
## Art tab — "Years in Motion" (2026-09-07)

A seventh nav view, **`("art", "Art")`**, appended after Exploratory. One entry for now.

Unlike every other view, this is **not a Plotly figure** — so `tidy_dark()` / `fig_html()` do not
apply to it. It is hand-built SVG at absolute user units, generated by
**`strava-data/dashboard/art_year.py`** and injected as a single `art_html` fragment (its own
`<style>`, `<svg>`, controls and `<script>`) by `_assemble_html()`.

### What it draws

Every activity of a calendar year in one figure, with a picker for each year present in the data
(currently 2024, 2025, 2026 — discovered from `activities.csv`, not hardcoded).
...
- **One scale shared across years** (longest activity in the whole file, and an 85th-percentile
  bloom extent). Per-year normalization would make a 5-mile run in a thin year draw as long as a
  13-mile hike in a heavy one — exactly the comparison the picker invites.
- **The angle denominator follows the year** (366 in a leap year), or 2024's Dec 31 lands
  somewhere 2025's does not.
- **Partial years are labeled** in the picker (amber dot) and in the center subtitle.
- **`max-width: 720px`** on the SVG: it is square, so at full card width it becomes ~1150px tall
  and the card turns into a scroll.
...
`dashboard/art_year.py` is the canonical generator. `tools/proof_year_art.py` **imports from it**
(adding `strava-data/` to `sys.path`) rather than keeping a second copy — the `feed/places.py`
duplication is the trap being avoided. That tool still owns the four exploration proofs
(grid / spiral / bloom / clock) and writes the proof sheet under
`Project Docs/Plans/strava-data/year-art/`.

### QA checks

- Tab bar shows seven tabs ending in **Art**; `#art` in the URL paints `#view-art` before the
  router runs (`view_paint_css` covers it automatically via `VIEW_NAMES`).
- `.art-year` groups exist for every year in the data; exactly one is visible.
- Spoke count per year equals that year's activity count — **including** zero-distance activities,
  which draw as ticks inside the ring rather than zero-length spokes that never appear.
