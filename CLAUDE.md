...
figures instead of a text legend. Those figures come from hand-made drawings, vectorised into
...
*filled outlines* (fill-rule evenodd), not stroked centrelines, so a glyph's line weight is baked
into its shape; the poster strokes each outline in its own colour to bring the ink up to the routes'
...
The six families exist to keep colour meaningful, so they are **not** Strava's enum: road and trail
...
for only one half of the downhill family, and for the rarer half. Merging a family for the legend must not empty the poster of a
