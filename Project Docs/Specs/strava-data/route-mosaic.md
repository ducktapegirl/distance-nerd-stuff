...
* Review the "route mosaic" idea, image, and related code:
	...
	* **strava-data/feed/places.py** — `all_tracks()` supplies the data: reads up to 400 stream CSVs, downsamples each to 64 points, and normalizes via `normalize()` so the dominant axis spans 0..1. It's `@lru_cache`'d because the full read is ~1.8s and several cards want it.
