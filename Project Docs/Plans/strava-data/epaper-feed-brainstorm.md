...
**Status:** prototype shipped (10 of 56 ideas built) · **Created:** 2026-09-03 · **Owner:** unassigned
...
56 ideas. **[BUILT]** marks the ten in the prototype. Every recipe below was checked against the real
data; the numbers quoted are live as of the 2026-08-30 fetch.

### A · Right now — state you would glance at

1. **[BUILT] Load gauge** — ACWR (7-day ÷ 28-day mean daily suffer score) on a four-band dial:
   ...
   *Today: **1.37**, spiking.* Bands get monotonically more ink as risk rises, so the ramp reads as
   ...
   the readout.
2. **Days since last activity** — one enormous numeral; the whole screen is the number. Trivial off
   `streaks()["days_since"]`.
3. **Last activity card** — sport glyph, name, distance, pace, elevation. The dashboard's
   `_activity_detail_json` already emits exactly these fields, pre-formatted.
4. **Rolling 7-day totals** — miles + hours + feet as three big numbers. `totals(window(acts, 7))`.
5. **Fresh / cooked** — a single word from ACWR + days-since, with a face-glyph. The most
   "fridge magnet" idea in the list.
6. **Last activity's route** — as #36 but pinned to the most recent GPS activity rather than rotating.

### B · Streaks and consistency

7. **Active-day streak** — consecutive days with an activity. *Current 1, longest ever 7.*
8. **Rest-day counter** — days since the last full rest day; the inverse framing, and for this
   athlete (355 active days of 687) the more interesting one.
9. **[BUILT] Last-30-days strip** — 30 cells, filled = active. *17 of 30.* Rendered as **two rows of
   15**, not one row of 30: at 800 px a single row forces 20 px cells (~2 mm) that vanish at arm's
   length.
10. **Week-shape bars** — 7 bars Mon–Sun, this week's miles against the 8-week median as a ghost outline.
11. **Consistency ratio** — active ÷ elapsed days, all-time (355/687 = 52 %) as a split disc.
12. **Day-of-week fingerprint** — 7 bars. *Sunday 70, Wednesday 55, Saturday 43.* Counterintuitive
    (Saturday is the **least** active day) and therefore worth a card.
13. **Longest streak vs current** — two bars racing. Only interesting when the gap is small.

### C · Volume and progress

14. **YTD vs the same date last year** — *536 mi vs 489 mi.* Two bars plus a delta pill.
15. **This month vs the 12-month median** — a thermometer that fills. *Aug 58 mi against a ~70 mi median.*
16. **[BUILT] Rolling 12-month odometer** — *880 mi*, digits in mechanical odometer boxes,
    `MILES · LAST 365 DAYS` beneath. Footer carries activities / feet / moving hours.
17. **Monthly sparkline** — 13 months of miles as a 3 px step line, no axis, a dot on "now".
18. **[BUILT] Elevation as landmark** — *126,355 ft = 4.4 × Everest*, drawn as four solid peaks plus
...
19. **[BUILT ×2] The Journey ladder** — see the expanded section below.
20. **Sport split** — last 365 days: *Run 70, MTB 68*, everything else trailing. Worth showing
    precisely because it is a dead heat.
21. **Hours-in-motion clock** — *285.9 moving hours* as a 12-hour dial wound N times round.

### D · The racing self — segments

22. **[BUILT] Latest PR** — segment name, time, date, effort number, and the effort count as
...
23. **PR pace** — PRs set in the last 30 / 90 / 365 days as three counters. *30 / 135 / 500.*
    Currently the footer of #22; deserves its own card.
24. **Home-segment leaderboard** — top 5 by effort count with best times, as a scoreboard.
    *Canyon entrance via Salix ×36, Salix out to PV ×29, Lenkeit bridge ×21.*
25. **Most-improving segment** — biggest negative `recent_trend` among segments with ≥5 efforts.
    *Lex Town Track, −40.7 %.*
26. **Most-declining segment** — the honest inverse. *Tree Y/T 1 split, +48.7 %.* Ships with a
    self-deprecating caption or it is just mean.
27. **Segment consistency spotlight** — reuse `rollups_cards.seg_consistency_picks`; show the
    coefficient of variation as a dot scatter of every effort.
28. **Repeat-offender counter** — one segment, its count as tally marks. The tally renderer from
    #22 already handles up to 40.
29. **The crossover fact** — reuse `chart_seg_grade_vs_time`: the grade at which running overtakes
    mountain biking. A one-sentence card, no chart.

### E · Gear — the most actionable category

30. **[BUILT] Shoe mileage bars** — one row per non-retired shoe: data-glyph, name, filled bar,
...
31. **[BUILT, merged into #30] Retire-me alert** — reversed out of the full black bar in white, so
    ...
    *ASICS DS Trainer at 470 mi against a 450 mi threshold — over.* The one genuinely **useful**
    card here.
32. **Bike odometer** — *Wile E. Coyote, 250.2 mi*, a bike glyph with a filling frame.
33. **Gear graveyard** — the retired ASICS and its description: *"last of its kind. I've purchased
    the same shoe for 15 years."* Pure personality, zero computation.

### F · Places

34. **Passport counter** — *28 regions, 9 states and provinces*, via the dashboard's
    `_count_regions` (10 km greedy clustering) and `_count_states` (39-box lat/lng table).
35. **Two homes** — *San Diego 782 mi vs Boston 530 mi*, two route thumbnails side by side.
    Reuse `_home_stats` and `_home_thumb_tracks`.
36. **[BUILT] Route of the day** — one activity's GPS path, chosen deterministically by date so it
...
37. **Route mosaic** — 24 thumbnails in a 6×4 grid. Abstract, and dense pixels are exactly what this
    panel is good at.
38. **Heat-map tile** — home-city route density, dithered rather than glowed.
39. **Compass extremes** — *northernmost 49.3°N, easternmost 70.2°W, highest 14,507 ft*, from the
    pinned `_PEAKS_DEF` record book.

### G · Weather and environment

40. **Temperature range** — trained from *−14.5 °C to 32.7 °C* (**6 °F to 91 °F**). A thermometer
    with two marks.
41. **The heat verdict** — reuse V4: pace degrades with heat, heart rate does not. One sentence.
42. **UV exposure** — *max 8.7*; a sun glyph whose ray count is the index. Note `uv_index` is
    time-of-day resolved, not a daily max (a 07:34 run reads 0.1).
43. **Dark o'clock** — *22 starts before 8 a.m., earliest 03:17.* A moon/sun split glyph.

### H · Records and superlatives

44. **Record book** — the six pinned `_PEAKS_DEF` rows, one per rotation day, each a full-screen fact.
45. **Longest ever** — longest run, longest ride, biggest climb day.
46. **Kudos leaderboard** — *"Snow Snake 🐍", 12 kudos.* Small and human.

### I · Memory

47. **On this day** — same month/day in prior years. **Sparse**: the dataset only spans 2024–2026,
    so most days have exactly one hit. Must degrade to "nothing on this day — here is the nearest".
48. **A year ago this week** — a wider, far more reliable window than #47. Prefer this one.
49. **First ever** — the first activity in the dataset, framed as an origin story.

### J · Voice and whimsy

50. **The joggernaut byline** — the Strava bio (*"I'm the joggernaut, bitch"*) as a masthead.
    Already the RSS channel description.
51. **[BUILT] From the logbook** — an activity title and its description, typographically, rotating
...
52. **Coyote index** — count descriptions matching an animal word list. Silly, cheap, and yours.
53. **Emoji-title census** — how many activity names are pure emoji.

### K · Meta and data-nerd

54. **Dataset stats** — *374 activities, 687,776 GPS points, 42 MB of streams.* A card about the data.
55. **Device timeline** — *Forerunner 255S ×364, 255 ×9, Strava App ×1.*
56. **Lap splits** — the first use of the **unconsumed `laps/` data** (374 files, 1,356 rows): the
...
## What the prototype ships

`strava-data/feed/` — `config.py` (panel constants, tone ramp), `metrics.py` (pure computation),
`journey.py`, `svg.py` (primitives, dither, glyphs, the `Card` container), `cards.py` (one function
per idea), `rss.py`, `page.py`. Entrypoint `strava-data/build_feed.py`; wired into `deploy.yml` after
the two existing builds. Ten cards, rotating daily by `date.toordinal() % 10`.
...
Build is stdlib + numpy, no network. RSS parses with 10 unique GUIDs, RFC-822 dates and no HTML in
descriptions. Rendered at exactly 800×480 in Chromium: minimum text 26 px, minimum effective stroke
3.00 px (accounting for `scale()` on glyph groups), scroll extent exactly 800×480, and no text
clipped past the card edge on any of the ten. Quantized to the panel's four tones every card stays
legible, with 4–18 % ink coverage. No metric display units (`km`, `km/h`, `°C`) in any output. Both
existing dashboards still build and `running-log/qa.py` passes 13/13.
...
3. **Bump the fetch cron to daily** if these cards should feel live.
